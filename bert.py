...
def generate_sentence_embedding(sentence,tokenizer,bert_model):
    ...
    token_embeddings = torch.squeeze(token_embeddings, dim=1)  #get rid of the “batches” dimension, [# layers, # tokens, # features]
    sentence_embedding = token_embeddings[-2, :, :]
    sentence_embedding = torch.sum(sentence_embedding,keepdims = True, dim=0)
    sentence_embedding = sentence_embedding.numpy()  # convert tensors to numpy array to fit the scikit-learn librar
